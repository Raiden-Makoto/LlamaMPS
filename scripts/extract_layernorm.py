...
def extract_layernorm(layer: int):
    print(f"Extracting Layer {layer} RMSNorm weights...")
    state_dict = load_file(MODEL_PATH)
    
    # Qwen-2.5-1.5B identifier for the first layer's norm
    norm_key = f"model.layers.{layer}.input_layernorm.weight"
    ...
    if norm_key in state_dict:
        ...
        norm_path = os.path.join(out_dir, f"layer{layer}_norm.bin")
